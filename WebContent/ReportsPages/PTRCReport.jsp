...
<html>
 <%
	...
	String QueryStringTable = "SELECT PTRC_Pkey,ser1.Usr_CompanyName,ser_yearSelect,SUM(PTRC_AmountPayable) AS totalPayable,SUM(PTRC_amountPaid1+PTRC_amountPaid2+PTRC_amountPaid3+PTRC_amountPaid4) AS total,COUNT(*) AS ReturnNos,PTRC_Type,PTRC_UserKey FROM PTRCservice vat INNER JOIN usrdetails ser1 ON vat.PTRC_UserKey=ser1.Usr_Id INNER JOIN serviceselector isr ON vat.PTRC_SerKey=isr.Ser_Id WHERE PTRC_Type='Orignal' AND isr.Ser_YearSelect='2008-09' GROUP BY PTRC_UserKey;"; 		
  ...
  <TABLE id ="customers" class="sortable" cellpadding="0" cellspacing="0" border="0">
	...
	<th class="sort-alpha"><h3>Total Return</h3></th>
			...
			for (ArrayList temp : Product){ 
				...
				<TD style="display: none;"><%=temp.get(8)%></td>
				</tr>
				<%i++;				
				}%>
			</tbody>
