...
BhanuSessionCheckerHelper help=new BhanuSessionCheckerHelper();
	...
	%>
		...
		if(id!=null)
		{
			...
			String QueryString = "SELECT Ser_id,Ser_UserId,ism.Usr_CompanyName,ism.Usr_PTRCNO,ism.Usr_FileNo,ism.Usr_ComapnyAllocation,ism.Usr_PTRCAllocated,ism.Usr_ConsultancyAllocation,Ser_Ptrcperiod,Ser_PTrCSelection,Ser_PtrcAudit,ser_yearSelect from serviceselector isr INNER JOIN usrdetails ism  on isr.Ser_UserId=ism.Usr_Id where Ser_id='"+id+"'";
		...
		userRegistrationHelper.fillDurinNull(lRegistrationBean);
			...
			String QueryStringTable = "SELECT PTRC_pkey,PTRC_UserKey,PTRC_Type,PTRC_Period,PTRC_AmountPayable,(PTRC_amountPaid1+PTRC_amountPaid2+PTRC_amountPaid3+PTRC_amountPaid4) as total,PTRC_AckNo,PTRC_AckNoDate,PTRC_Status	FROM PTRCService vat	INNER JOIN serviceselector ser ON vat.PTRC_SerKey=ser.Ser_Id WHERE PTRC_UserKey='"+lRegistrationBean.getBagNo()+"' AND ser.Ser_YearSelect='"+lRegistrationBean.getWorkPhone()+"'and PTRC_type='Orignal'"; 		
			...
			String QueryStringTable1 = "SELECT PTRC_pkey,PTRC_UserKey,PTRC_Type,PTRC_Period,PTRC_AmountPayable,(PTRC_amountPaid1+PTRC_amountPaid2+PTRC_amountPaid3+PTRC_amountPaid4) as total,PTRC_AckNo,PTRC_AckNoDate,PTRC_Status	FROM PTRCService vat	INNER JOIN serviceselector ser ON vat.PTRC_SerKey=ser.Ser_Id WHERE PTRC_UserKey='"+lRegistrationBean.getBagNo()+"' AND ser.Ser_YearSelect='"+lRegistrationBean.getWorkPhone()+"'and PTRC_type='Revised'"; 		
...
{
		var reason="";	
		reason += validateEmpty(PTRCForm.PTRCRemarks);
		reason += validateEmpty(PTRCForm.PTRCStatus);
		reason += validateEmpty(PTRCForm.AmountPayable);
		reason += validateEmpty(PTRCForm.PTRCPeriod);
		reason += validateEmpty(PTRCForm.PTRCType);
		reason += validateEmpty(PTRCForm.AckNo);
		reason += validateEmpty(PTRCForm.AckNoDate);
		reason += validateCheckDate(PTRCForm.pre_AmountPaid1,PTRCForm.AmountPaidDate1);
		reason += validateCheckDate(PTRCForm.pre_AmountPaid2,PTRCForm.AmountPaidDate2);
		reason += validateCheckDate(PTRCForm.pre_AmountPaid3,PTRCForm.AmountPaidDate3);
		reason += validateCheckDate(PTRCForm.pre_AmountPaid4,PTRCForm.AmountPaidDate4);
		reason += validateCheckDate(PTRCForm.AckNo,PTRCForm.AckNoDate);
		if (reason != "") 
		{
		alert("Some fields need correction:\n" + reason);
			return false;
		} else {
			document.PTRCForm.submit();
			return true;
		}
	}
...
<body >
 ...
 <br>
 <div id="clean"></div>
 <form name="PTRCForm" id="PTRCForm" style="padding-top: 0px; margin-top: 0px;"action="<%=request.getContextPath()%>/PTRCFormRegistration" method="post"onsubmit="javascript:return false;">
			...
			<tr>
			<td class="Title">PTRC Person Allocation   </td>
			...
			<td class="Title">PTRC Periodicity   </td>
...
<br>		
	...
	<th class="sort-alpha"><h3>Status</h3></th><!-- not required -->
		...
		<table id="yrselection" cellspacing="5" cellpadding="0" width="90%">
			...
			<td>
			<select id="PTRCPeriod" name="PTRCPeriod">
			...
			<td>
			<select id="PTRCStatus" name="PTRCStatus">
				<option value="">select</option>
			...
			<td colspan="4"><textarea id="PTRCRemarks" rows="2" cols="80"></textarea>
			</tr>
			<tr>
			<td></td>
